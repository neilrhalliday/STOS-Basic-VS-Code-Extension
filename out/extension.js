...
function activate(context) {
    ...
    const provider1 = vscode.languages.registerCompletionItemProvider('stos', {
        provideCompletionItems(document, position, token, context) {
            ...
            Completion_max.documentation = new vscode.MarkdownString('_x_=**MAX**(_y_,_z_)\n\n**MAX** compares two expressions (_y_ & _z_) and returns the largest. These expressions can be composed of numbers or strings, providing you don\'t try to mix different types of expression in one instruction.\n\n_***Examples:***_\n\nprint max(10,4)   \n10   \n\nprint max("Hello","Hi")   \nHi  \n\nprint max(10,"Hi")  \nType mismatch\n\nSee **MIN**');
            Completion_max.kind = vscode.CompletionItemKind.Function;
            const Completion_min = new vscode.CompletionItem('min');
            Completion_min.documentation = new vscode.MarkdownString('_x_=**MIN**(_y_,_z_)\n\n**MIN** compares two expressions (_y_ & _z_) and returns the smallest. These expressions can be composed of numbers or strings, providing you don\'t try to mix different types of expression in one instruction.\n\n_***Examples:***_\n\nprint min(10,4)   \n4   \n\nprint max("Hello","Hi")   \nHello  \n\nprint max(10,"Hi")  \nType mismatch\n\nSee **MAX**');
            Completion_min.kind = vscode.CompletionItemKind.Function;
            const Completion_swap = new vscode.CompletionItem('swap');
            Completion_swap.documentation = new vscode.MarkdownString('**SWAP**(_x_,_y_)\n\n**SWAP** exchanges the data between two variables of the same type.\n\n_***Example:***_\n\n10 A=1 : B=2   \n20 print A,B   \n30 swap(a,b)   \n40 print A,B');
            Completion_swap.kind = vscode.CompletionItemKind.Function;
            const Completion_deffn = new vscode.CompletionItem('def fn');
            Completion_deffn.documentation = new vscode.MarkdownString('**DEF FN** _name_ (_variablelist_)=_expression_\n\n**DEF FN** is a useful function which enables you to create your own user-defined functions for use within a STOS Basic program.\n\n_name_ is the name of the function you wish to define. _variablelist_ can be any list of variables seperated by commas. These variables are local to the function. Any variables you automatically use in the function will be substituted for the appropriate local variable whenever necessary. Also note that variables of different types can be mixed within a single function.\n\n_***Examples:***_\n\ndef fn SQ(X)=X+X   \ndef fn SEGMENT (A$,X,Y)=mid$(A$,X,Y)');
            Completion_deffn.kind = vscode.CompletionItemKind.Function;
            const Completion_fn = new vscode.CompletionItem('fn');
            Completion_fn.documentation = new vscode.MarkdownString('**FN** _name_\n\n**FN** is used to execute a user-defined function defined with the **DEF FN** command.\n\n_***Examples:***_\n\nfn SQ(5)   \nfn SEGMENT ("Hello",2,3)  \n\nSee **DEF FN**');
            Completion_fn.kind = vscode.CompletionItemKind.Function;
            const Completion_rnd = new vscode.CompletionItem('rnd');
            Completion_rnd.documentation = new vscode.MarkdownString('_x_=**RND**(_y_)\n\n**RND** is used to generate a random integer between 0 and _y_ inclusive. If _y_ is less than zero, **RND** will return the last value it produced.\n\n_***Examples:***_\n\n10 plot rnd(640/divx-1),rnd(400/divy-1)   \n20 goto 10   \n\nprint "Dice throw is a ",rnd(5)+1');
            Completion_rnd.kind = vscode.CompletionItemKind.Function;
            const Completion_fix = new vscode.CompletionItem('fix');
            Completion_fix.documentation = new vscode.MarkdownString('**FIX**(_n_)\n\n**FIX** sets the precision of any real numbers which are to be printed on the screen. There are three possibilities.\n\nif 0<_n_<16 then _n_ denotes the number of figures to be output after the decimal point.\n\nif _n_>16 the printout will be proportional and any trailing zeros will be removed.\n\nif _n_<0 then all floating point numbers will be displayed in exponential format.\n\n_***Examples:***_\n\nfix(2) : print pi   \nfix(-4) : print pi  \nfix(16) : print pi   ');
            Completion_fix.kind = vscode.CompletionItemKind.Function;
            // String Handling
            //
            const Completion_asc = new vscode.CompletionItem('asc');
            Completion_asc.documentation = new vscode.MarkdownString('_x_=**ASC**(_a$_)\n\n**ASC** returns the Ascci code of the first character of the string in _A$_.\n\n_***Example:***_\n\nprint asc("B")   \n66');
            Completion_asc.kind = vscode.CompletionItemKind.Function;
            const Completion_len = new vscode.CompletionItem('len');
            Completion_len.documentation = new vscode.MarkdownString('_x_=**LEN**(_a$_)\n\n**LEN** calculates the current length of a string of characters held in _a$_. All the characters of the string are counted, even if they are not visible on the screen.\n\n_***Example:***_\n\nprint len("12345678")   \n8   \n\nDo not confuse with **LENGTH**.');
            Completion_len.kind = vscode.CompletionItemKind.Function;
            const Completion_val = new vscode.CompletionItem('val');
            Completion_val.documentation = new vscode.MarkdownString('_x_=**VAL**(_a$_)\n\n**VAL** returns the value of a number that is stored in the string _a$_. If _a$_ does not contain a number then **VAL** will return zero.\n\n_***Example:***_\n\nprint val("66")   \n66');
            Completion_val.kind = vscode.CompletionItemKind.Function;
            const Completion_upper$ = new vscode.CompletionItem('upper$');
            Completion_upper$.documentation = new vscode.MarkdownString('_x$_=**UPPER$**(_a$_)\n\n**UPPER$** converts the string in _a$_ into upper case characters.\n\n_***Example:***_\n\nprint upper$("Hello")   \nHELLO   \n\nDo not confuse this with the editor command upper.');
            Completion_upper$.kind = vscode.CompletionItemKind.Function;
            const Completion_lower$ = new vscode.CompletionItem('lower$');
            Completion_lower$.documentation = new vscode.MarkdownString('_x$_=**LOWER$**(_a$_)\n\n**LOWER$** converts the string in _a$_ into lower case characters.\n\n_***Example:***_\n\nprint lower$("STOS Basic")   \nstos basic   \n\nDo not confuse this with the editor command lower.');
            Completion_lower$.kind = vscode.CompletionItemKind.Function;
            const Completion_flip$ = new vscode.CompletionItem('flip$');
            Completion_flip$.documentation = new vscode.MarkdownString('_x$_=**FLIP$**(_a$_)\n\n**FLIP$** reverses the order of the characters in the string _a$_.\n\n_***Example:***_\n\nprint flip$("STOS Basic")   \ncisaB SOTS');
            Completion_flip$.kind = vscode.CompletionItemKind.Function;
            const Completion_space$ = new vscode.CompletionItem('space$');
            Completion_space$.documentation = new vscode.MarkdownString('_x$_=**SPACE$**(_n_)\n\n**SPACE$** creates a string containing _n_ spaces.\n\n_***Example:***_\n\nA$ = space$(20) : print len(A$)   \n20');
            Completion_space$.kind = vscode.CompletionItemKind.Function;
            const Completion_string$ = new vscode.CompletionItem('string$');
            Completion_string$.documentation = new vscode.MarkdownString('_x$_=**STRING$**(_a$_,_n_)\n\n**STRING$** creates a string of _n_ characters in length using the first character of string _a$_.\n\n_***Example:***_\n\nprint string$("The cat sat on the mat",10)   \nTTTTTTTTTT   ');
            Completion_string$.kind = vscode.CompletionItemKind.Function;
            const Completion_chr$ = new vscode.CompletionItem('chr$');
            Completion_chr$.documentation = new vscode.MarkdownString('_x$_=**CHR$**(_n_)\n\n**CHR$** creates a string containing the character with the Ascci code _n_.\n\n_***Example:***_\n\nprint chr$(65)   \nB');
